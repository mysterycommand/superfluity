// tslint:disable variable-name

import { b2_toiSlop } from '../../common/settings';
import Shape from './shape';
import { MulX, Dot } from '../../common/math';
import Vec2 from '../../common/math/vec2';
import RayCastOutput from '../ray-cast-output';
import RayCastInput from '../ray-cast-input';
import Transform from '../../common/math/transform';
import Aabb from '../aabb';
import MassData from './mass-data';

export default class EdgeShape extends Shape {
  public s_supportVec = new Vec2();

  public m_v1 = new Vec2();
  public m_v2 = new Vec2();

  public m_coreV1 = new Vec2();
  public m_coreV2 = new Vec2();

  public m_normal = new Vec2();
  public m_direction = new Vec2();

  public m_cornerDir1 = new Vec2();
  public m_cornerDir2 = new Vec2();

  public m_length = 0;

  public m_cornerConvex1 = false;
  public m_cornerConvex2 = false;

  public m_prevEdge?: any;
  public m_nextEdge?: any;

  constructor(v1 = new Vec2(), v2 = new Vec2()) {
    super();

    this.m_type = Shape.e_edgeShape;

    this.m_prevEdge = undefined;
    this.m_nextEdge = undefined;

    this.m_v1 = v1;
    this.m_v2 = v2;

    this.m_direction.Set(this.m_v2.x - this.m_v1.x, this.m_v2.y - this.m_v1.y);
    this.m_length = this.m_direction.Normalize();

    this.m_normal.Set(this.m_direction.y, -this.m_direction.x);

    this.m_coreV1.Set(
      -b2_toiSlop * (this.m_normal.x - this.m_direction.x) + this.m_v1.x,
      -b2_toiSlop * (this.m_normal.y - this.m_direction.y) + this.m_v1.y,
    );
    this.m_coreV2.Set(
      -b2_toiSlop * (this.m_normal.x + this.m_direction.x) + this.m_v2.x,
      -b2_toiSlop * (this.m_normal.y + this.m_direction.y) + this.m_v2.y,
    );

    this.m_cornerDir1 = this.m_normal;
    this.m_cornerDir2.Set(-this.m_normal.x, -this.m_normal.y);
  }

  public RayCast(
    output: RayCastOutput,
    input: RayCastInput,
    transform: Transform,
  ) {
    const tMat = transform.R;
    const rX = input.p2.x - input.p1.x;
    const rY = input.p2.y - input.p1.y;

    const v1X =
      transform.position.x +
      (tMat.col1.x * this.m_v1.x + tMat.col2.x * this.m_v1.y);
    const v1Y =
      transform.position.y +
      (tMat.col1.y * this.m_v1.x + tMat.col2.y * this.m_v1.y);

    const nX =
      transform.position.y +
      (tMat.col1.y * this.m_v2.x + tMat.col2.y * this.m_v2.y) -
      v1Y;
    const nY = -(
      transform.position.x +
      (tMat.col1.x * this.m_v2.x + tMat.col2.x * this.m_v2.y) -
      v1X
    );

    const k_slop = 100.0 * Number.MIN_VALUE;
    const denom = -(rX * nX + rY * nY);
    if (denom > k_slop) {
      const bX = input.p1.x - v1X;
      const bY = input.p1.y - v1Y;
      let a = bX * nX + bY * nY;

      if (0.0 <= a && a <= input.maxFraction * denom) {
        const mu2 = -rX * bY + rY * bX;

        if (-k_slop * denom <= mu2 && mu2 <= denom * (1.0 + k_slop)) {
          a /= denom;

          output.fraction = a;
          const nLen = Math.sqrt(nX * nX + nY * nY);
          output.normal.x = nX / nLen;
          output.normal.y = nY / nLen;

          return true;
        }
      }
    }

    return false;
  }

  public ComputeAABB(aabb: Aabb, transform: Transform) {
    const tMat = transform.R;

    const v1X =
      transform.position.x +
      (tMat.col1.x * this.m_v1.x + tMat.col2.x * this.m_v1.y);
    const v1Y =
      transform.position.y +
      (tMat.col1.y * this.m_v1.x + tMat.col2.y * this.m_v1.y);

    const v2X =
      transform.position.x +
      (tMat.col1.x * this.m_v2.x + tMat.col2.x * this.m_v2.y);
    const v2Y =
      transform.position.y +
      (tMat.col1.y * this.m_v2.x + tMat.col2.y * this.m_v2.y);

    if (v1X < v2X) {
      aabb.lowerBound.x = v1X;
      aabb.upperBound.x = v2X;
    } else {
      aabb.lowerBound.x = v2X;
      aabb.upperBound.x = v1X;
    }

    if (v1Y < v2Y) {
      aabb.lowerBound.y = v1Y;
      aabb.upperBound.y = v2Y;
    } else {
      aabb.lowerBound.y = v2Y;
      aabb.upperBound.y = v1Y;
    }
  }

  public ComputeMass(massData: MassData, density = 0) {
    massData.mass = 0;
    massData.center.SetV(this.m_v1);
    massData.I = 0;
  }

  public ComputeSubmergedArea(
    normal: Vec2,
    offset = 0,
    xf: Transform,
    c: Vec2,
  ) {
    const v0 = new Vec2(normal.x * offset, normal.y * offset);

    const v1 = MulX(xf, this.m_v1);
    const v2 = MulX(xf, this.m_v2);

    const d1 = Dot(normal, v1) - offset;
    const d2 = Dot(normal, v2) - offset;

    if (d1 > 0) {
      if (d2 > 0) {
        return 0;
      } else {
        v1.x = (-d2 / (d1 - d2)) * v1.x + (d1 / (d1 - d2)) * v2.x;
        v1.y = (-d2 / (d1 - d2)) * v1.y + (d1 / (d1 - d2)) * v2.y;
      }
    } else {
      if (d2 > 0) {
        v2.x = (-d2 / (d1 - d2)) * v1.x + (d1 / (d1 - d2)) * v2.x;
        v2.y = (-d2 / (d1 - d2)) * v1.y + (d1 / (d1 - d2)) * v2.y;
      }
    }

    c.x = (v0.x + v1.x + v2.x) / 3;
    c.y = (v0.y + v1.y + v2.y) / 3;

    return (
      0.5 * ((v1.x - v0.x) * (v2.y - v0.y) - (v1.y - v0.y) * (v2.x - v0.x))
    );
  }

  public GetLength() {
    return this.m_length;
  }

  public GetVertex1() {
    return this.m_v1;
  }

  public GetVertex2() {
    return this.m_v2;
  }

  public GetCoreVertex1() {
    return this.m_coreV1;
  }

  public GetCoreVertex2() {
    return this.m_coreV2;
  }

  public GetNormalVector() {
    return this.m_normal;
  }

  public GetDirectionVector() {
    return this.m_direction;
  }

  public GetCorner1Vector() {
    return this.m_cornerDir1;
  }

  public GetCorner2Vector() {
    return this.m_cornerDir2;
  }

  public Corner1IsConvex() {
    return this.m_cornerConvex1;
  }

  public Corner2IsConvex() {
    return this.m_cornerConvex2;
  }

  public GetFirstVertex(xf: Transform) {
    const tMat = xf.R;

    return new Vec2(
      xf.position.x +
        (tMat.col1.x * this.m_coreV1.x + tMat.col2.x * this.m_coreV1.y),
      xf.position.y +
        (tMat.col1.y * this.m_coreV1.x + tMat.col2.y * this.m_coreV1.y),
    );
  }

  public GetNextEdge() {
    return this.m_nextEdge;
  }

  public GetPrevEdge() {
    return this.m_prevEdge;
  }

  public Support(xf: Transform, dX = 0, dY = 0) {
    const tMat = xf.R;

    const v1X =
      xf.position.x +
      (tMat.col1.x * this.m_coreV1.x + tMat.col2.x * this.m_coreV1.y);
    const v1Y =
      xf.position.y +
      (tMat.col1.y * this.m_coreV1.x + tMat.col2.y * this.m_coreV1.y);

    const v2X =
      xf.position.x +
      (tMat.col1.x * this.m_coreV2.x + tMat.col2.x * this.m_coreV2.y);
    const v2Y =
      xf.position.y +
      (tMat.col1.y * this.m_coreV2.x + tMat.col2.y * this.m_coreV2.y);

    if (v1X * dX + v1Y * dY > v2X * dX + v2Y * dY) {
      this.s_supportVec.x = v1X;
      this.s_supportVec.y = v1Y;
    } else {
      this.s_supportVec.x = v2X;
      this.s_supportVec.y = v2Y;
    }

    return this.s_supportVec;
  }

  public SetPrevEdge(edge: any, core: Vec2, cornerDir: Vec2, convex: boolean) {
    this.m_prevEdge = edge;
    this.m_coreV1 = core;
    this.m_cornerDir1 = cornerDir;
    this.m_cornerConvex1 = convex;
  }

  public SetNextEdge(edge: any, core: Vec2, cornerDir: Vec2, convex: boolean) {
    this.m_nextEdge = edge;
    this.m_coreV2 = core;
    this.m_cornerDir2 = cornerDir;
    this.m_cornerConvex2 = convex;
  }
}
